...
class AuthService extends ChangeNotifier {
  ...
  Future<void> _loadFromStorage() async {
    try {
      ...
      if (userId != null) {
        ...
        try {
          final dname = await _getDeviceName();
          final dinfo = await _getDeviceInfo();
  ...
  Future<void> updateLastActive() async {
    try {
      ...
      // verify token and device presence on server
      if (userId != null) {
        try {
          await api.getUserById(userId!);
          final devices = await api.getUserDevices(userId!);
          final found = devices.any((d) {
            final devId = (d['device_id'] ?? d['device_uuid'] ?? d['id'] ?? '')
                .toString();
  ...
  Future<String> _getDeviceName() async {
    try {
      ...
      if (defaultTargetPlatform == TargetPlatform.android) {
        ...
        final m = info.manufacturer.toString();
        final mo = info.model.toString();
        return '$m $mo'.trim();
      }
      if (defaultTargetPlatform == TargetPlatform.iOS) {
        final info = await plugin.iosInfo;
        final nm = info.name.toString();
        final machine = info.utsname.machine.toString();
  ...
  Future<String> _getDeviceInfo() async {
    try {
      ...
      if (defaultTargetPlatform == TargetPlatform.android) {
        final info = await plugin.androidInfo;
        final b = info.brand.toString();
        final mo = info.model.toString();
        final rel = info.version.release.toString();
        return '$b $mo $rel'.trim();
      }
      if (defaultTargetPlatform == TargetPlatform.iOS) {
        final info = await plugin.iosInfo;
        final s = info.systemName.toString();
        final sv = info.systemVersion.toString();
        return '$s $sv'.trim();
      }
      return '';
    } catch (e) {
